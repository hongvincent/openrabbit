...
1. ``.openrabbit.yaml`` — config-as-code with sensible defaults derived from the
   detected stack (lenses + ``model_roles`` defaulting to a Nova 2 Lite finder and
   a GPT-5.4 verifier per the live-verified routing, SPEC §7.2). ``external_tools``
   is scaffolded as a RESERVED
   block (``enabled: []`` + a comment) — the pipeline does not run those graders
   yet, so the config never advertises a feature that does not execute.
...
#: places the GPT-5.4 verifier in us-east-2).
DEFAULT_AWS_REGION = "us-east-2"

#: The secret name the caller workflow passes through to the reusable workflow.
AWS_ROLE_ARN_SECRET = "AWS_ROLE_ARN"

#: ``external_tools`` is RESERVED: the pipeline does not yet run these graders or
#: inject their output into the review context. The scaffold therefore ships
#: ``external_tools.enabled: []`` (the detected graders are surfaced only as a
#: comment / in :attr:`DetectedStack.external_tools` for when the plumbing lands),
#: so a generated config never advertises a feature that does not run. Flip this
#: to ``False`` and populate ``enabled`` only once the graders are actually wired.
EXTERNAL_TOOLS_RESERVED = True
...
class DetectedStack:
    ...
    ``external_tools`` is the deterministic-grader allow-list detected for the
    stack. NOTE: it is RESERVED / not yet wired — the scaffold lists it only in a
    comment and ships ``external_tools.enabled: []`` (see
    :data:`EXTERNAL_TOOLS_RESERVED`); the harness does not run these graders yet.
    """
...
def _render_config_yaml(stack: DetectedStack) -> str:
    ...
    tests (it round-trips). ``model_roles`` default to a Nova 2 Lite finder/triage
    (Seoul) + a GPT-5.4 verifier (us-east-2) per the live-verified routing
    (SPEC §7.2); these match the registry, pricing, and ``.openrabbit.example.yaml``.

    ``external_tools`` is scaffolded as a **reserved, not-yet-wired** block: the
    detected graders are listed in a comment for when the plumbing lands, but
    ``enabled`` ships EMPTY so the config never advertises a feature the pipeline
    does not yet execute (the harness does not run these tools or inject their
    output today). See :data:`openrabbit.init.EXTERNAL_TOOLS_RESERVED`.
    """
    detected = stack.external_tools or ["gitleaks"]
    detected_hint = ", ".join(detected)
    return f"""\
...
# model/inference-profile ids. Defaults follow the live-verified routing (SPEC
# §7.2): a Nova 2 Lite finder/triage (Seoul) + a GPT-5.4 verifier (us-east-2).
...
# external_tools is RESERVED / not yet wired: openrabbit does not run these
# graders or inject their output into the review yet. Keep `enabled` empty so the
# config never advertises a feature the pipeline does not execute. Detected for
# this stack (uncomment once the plumbing lands): {detected_hint}
external_tools:
  enabled: []
